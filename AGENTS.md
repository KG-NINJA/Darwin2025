...
Read `README.md` and `.github/workflows/evolve.yml` when working on the evolution
loop. Inspect only relevant portions of `agents/child_1/evol.md` and state when
needed; preserve prior logs, generated-code history and current state. Use
Japanese for meaningful new comments and evolution notes.
The existing evolve workflow is scheduled/manual and performs model calls,
generation and commits. It is not a development test; do not dispatch, resume
or change it merely to validate instructions. No deployment route is tracked.
No package.json, Jest, Storybook, pixel-progress script or kgninja_agent package
is present; do not require those former example commands. For changes to the
setup script, `python -m py_compile setup_evol_loop.py` checks syntax only.
Use relevant behavior checks for code changes and report unavailable coverage.
Do not generate progress art or impose arbitrary 60-second/three-file cutoffs;
follow the requested scope and actual validation results.
